...
void SiStripThresholdDQM::bookHistos(DQMStore::IBooker & ibooker){
  // Build the Histo_TkMap:
  if(HistoMaps_On_ ){
    if(WhichThreshold=="Low") Tk_HM_L = new TkHistoMap(ibooker,"SiStrip/Histo_Map","LowThresh_TkMap",0.);
    if(WhichThreshold=="High") Tk_HM_H = new TkHistoMap(ibooker,"SiStrip/Histo_Map","HighThresh_TkMap",0.);
  }
}
