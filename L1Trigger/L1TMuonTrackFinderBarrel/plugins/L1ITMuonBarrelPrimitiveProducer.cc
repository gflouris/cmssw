...
private:
  ...
  const PrimitiveCombiner::resolutions _resol;
...
std::ostream & operator<< (std::ostream & out, const L1TMuon::TriggerPrimitiveList & rpc )
{
  std::vector<L1TMuon::TriggerPrimitiveRef>::const_iterator it = rpc.begin();
  std::vector<L1TMuon::TriggerPrimitiveRef>::const_iterator end = rpc.end();
					      ...
					      const edm::EventSetup& iSetup )
{
  ...
  edm::Handle<L1TMuon::MBLTContainer> mbltContainer;
  iEvent.getByLabel( _mbltCollectionInput, mbltContainer );

  L1TMuon::MBLTContainer::const_iterator st = mbltContainer->begin();
  L1TMuon::MBLTContainer::const_iterator stend = mbltContainer->end();
  ...
  for ( ; st != stend; ++st ) {
    ...
    const L1TMuon::MBLTCollection & mbltStation = st->second;
    ...
    for ( size_t iDt = 0; iDt < dtListSize; ++iDt ) {
      const L1TMuon::TriggerPrimitiveRef & dt = mbltStation.getDtSegments().at(iDt);
    ...
    for ( size_t idxDt = 0; idxDt < cSize; ++idxDt ) {
      ...
      if ( iDt < 0 ) continue;
      const L1TMuon::TriggerPrimitive & dt = *mbltStation.getDtSegments().at(iDt);
      L1TMuon::TriggerPrimitiveList rpcInMatch = mbltStation.getRpcInAssociatedStubs( iDt );
      L1TMuon::TriggerPrimitiveList rpcOutMatch = mbltStation.getRpcOutAssociatedStubs( iDt );
      size_t rpcInMatchSize = rpcInMatch.size();
      size_t rpcOutMatchSize = rpcOutMatch.size();
      if ( rpcInMatchSize && rpcOutMatchSize ) {
	const L1TMuon::TriggerPrimitive & rpcIn = *rpcInMatch.front();
	const L1TMuon::TriggerPrimitive & rpcOut = *rpcOutMatch.front();
	...
	const L1TMuon::TriggerPrimitiveRef & dtM = mbltStation.getDtSegments().at(jDt);
	...
	if ( deltaPhiDt < minDeltaPhiDt ) {
      ...
      L1TMuon::MBLTCollection::bxMatch match = L1TMuon::MBLTCollection::NOMATCH;
	...
	match = mbltStation.haveCommonRpc( iDt, closest );
      ...
      if ( match != L1TMuon::MBLTCollection::NOMATCH ) {
	...
	/// there is a pair HI+HO with a shared inner RPC hit
	if ( match == L1TMuon::MBLTCollection::INMATCH ) {
	...
	} else if ( match == L1TMuon::MBLTCollection::OUTMATCH ) {
	...
	} else if ( match == L1TMuon::MBLTCollection::FULLMATCH ) {
...
//     for ( size_t out = 0; out < rpcOutUSize; ++out ) {
    ...
    const std::vector< std::pair< L1TMuon::TriggerPrimitiveList, L1TMuon::TriggerPrimitiveList > >
    ...
    for ( ; rpcPair != rpcPairEnd; ++ rpcPair ) {
      const L1TMuon::TriggerPrimitiveList & inRpc = rpcPair->first;
      const L1TMuon::TriggerPrimitiveList & outRpc = rpcPair->second;
	...
	// const L1TMuon::TriggerPrimitive & rpc = (*inRpc.at(inPos));
	L1TMuon::TriggerPrimitive rpc = (*inRpc.at(inPos));
	...
	double avPhiOut = atan2( avPhiSin, avPhiCos );
        ...
        // const L1TMuon::TriggerPrimitive & rpc = (*outRpc.at(outPos));
	L1TMuon::TriggerPrimitive rpc = (*outRpc.at(outPos));
...
//           }
//         }
//         const L1TMuon::TriggerPrimitive & rpc_in = (*inRpc.at(inPos));

//         const L1TMuon::TriggerPrimitive & rpc_out = (*outRpc.at(outPos));
