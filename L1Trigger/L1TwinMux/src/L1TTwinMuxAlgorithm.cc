...
void L1TwinMuxAlgortithm::run(
  ...
  rpc_dt_bx->run(c);
  L1MuDTChambPhContainer phiDigisBxRPC = rpc_dt_bx->getDTContainer();
    ...
    for (wheel=-2;wheel<=2; wheel++ ){
      for (sector=0;sector<12; sector++ ){
        for (station=1; station<=4; station++){
          ...
          dtts1 = phiDigisBxRPC.chPhiSegm1(wheel,station,sector,bx);
          dtts2 = phiDigisBxRPC.chPhiSegm2(wheel,station,sector,bx - 1 );
...
m_tm_phi_output.setContainer(l1ttma_out);
  ...
  if(verbose){
     ...
     cout<<"======RPC========"<<endl;
     ioPrinter.run(&rpcPhiDigis, m_tm_phi_output, rpcDigis, c);
  }
